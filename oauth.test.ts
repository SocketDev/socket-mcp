  ...
  const server = createServer(
    async (req: IncomingMessage, res: ServerResponse) => {
      ...
      if (req.method === 'POST' && url.pathname === '/introspect') {
        ...
        const introspectionResponse = token
          ? mockIntrospectionResponses[token]
          : undefined
...
async function waitForHealth(
  ...
  while (Date.now() < timeoutAt) {
    if (child.exitCode !== null) {
      throw new Error(
        `HTTP server exited before becoming ready:\n${getOutput()}`,
      )
    }
  ...
  throw new Error(
    `Timed out waiting for HTTP server readiness:\n${getOutput()}`,
  )
}
...
test('HTTP OAuth metadata and auth semantics', async t => {
  ...
  await t.test(
    'does not expose upstream authorization server metadata',
    async () => {
  ...
  await t.test(
    'serves protected resource metadata pointing to the issuer',
    async () => {
  ...
  await t.test(
    'ignores forwarded host and proto headers unless TRUST_PROXY is enabled',
    async () => {
      const response = await fetch(
        `${server.baseUrl}${protectedResourceMetadataPath}`,
        {
  ...
  await t.test(
    'returns invalid_request when the Authorization header is missing',
    async () => {
  ...
  await t.test(
    'returns invalid_token when introspection reports an inactive token',
    async () => {
  ...
  await t.test(
    'returns insufficient_scope when the token is missing required scopes',
    async () => {
      const response = await fetch(`${server.baseUrl}/`, {
        method: 'POST',
        headers: {
  ...
  await t.test(
    'accepts an active token even when introspection omits exp',
    async () => {
      const response = await fetch(`${server.baseUrl}/`, {
        method: 'POST',
        headers: {
...
test('TRUST_PROXY enables forwarded host and proto for OAuth metadata URLs', async t => {
  ...
  await assertOAuthErrorResponse(
