 ...
 *
 * Intercepts package install commands across npm, PyPI, Cargo, RubyGems,
 * Go, and NuGet, and checks the target package against the public Socket
 * MCP server. Blocks installs when the supply chain score is below 20
 ...
 *
 * Fails open on network, parse, and timeout errors so a Socket outage
 * does not block legitimate work.
 */
...
const INSTALL_PATTERNS: Array<{ ecosystem: Ecosystem, pattern: RegExp }> = [
  { ecosystem: 'npm', pattern: /\bnpm\s+(?:install|i|add)\s+([^\s-][^\s]*)/i },
  ...
  { ecosystem: 'pypi', pattern: /(?:\bpython3?\s+-m\s+)?\bpip3?\s+install\s+([^\s-][^\s]*)/i },
  ...
  { ecosystem: 'cargo', pattern: /\bcargo\s+(?:add|install)\s+([^\s-][^\s]*)/i },
...
function stripVersion (pkg: string, ecosystem: Ecosystem): string {
  switch (ecosystem) {
    case 'npm':
...
export function extractPackage (command: string): { ecosystem: Ecosystem, name: string } | null {
...
export function parseSupplyChainScore (text: string): number | null {
...
export async function checkPackage (
  ...
  if (payload.result?.isError) {
    throw new Error('Socket MCP reported a tool error (package likely not found)')
  }
...
const isMainModule = argv[1] !== undefined && fileURLToPath(import.meta.url) === argv[1]
