// node --test specs for the answer-questions-nudge hook.
//
// Stop hook (no tool_name/tool_input — it reads transcript_path).
// It's a -nudge: it never blocks (always exits 0); when it fires it
...
function makeTranscript(userText: string, assistantText: string): string {
  const dir = mkdtempSync(path.join(tmpdir(), 'answer-questions-nudge-'))
...
test('fires: question mark + "right now I\'m" deflection', async () => {
  ...
  assert.match(result.stderr, /answer-questions-nudge/)
