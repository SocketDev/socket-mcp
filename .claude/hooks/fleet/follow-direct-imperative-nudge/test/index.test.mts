// node --test specs for follow-direct-imperative-nudge.
...
test('fires: imperative user + hedging assistant', () => {
  ...
  assert.match(stderr, /follow-direct-imperative-nudge/)
})

test('silent: imperative user + clean assistant', () => {
  const { stderr } = runWithTurns('kill it', 'Done. Killed the process.')
  assert.doesNotMatch(stderr, /follow-direct-imperative-nudge/)
})
