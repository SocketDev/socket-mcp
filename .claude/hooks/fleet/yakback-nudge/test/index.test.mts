// node --test specs for the merged yakback-nudge hook. Asserts each
...
test('fires the comment-tone group', () => {
  ...
  try {
    ...
    assert.match(stderr, /comment-yakback-nudge/)
  } finally {
    cleanup()
  }
})
...
test('fires the identifying-users group', () => {
  ...
  try {
    ...
    assert.match(stderr, /identifying-users-nudge/)
  } finally {
    cleanup()
  }
})
...
test('fires the perfectionist group', () => {
  ...
  try {
    const { stderr } = runHook(p)
    assert.match(stderr, /perfectionist-nudge/)
  } finally {
    cleanup()
  }
})
...
test('fires the self-narration group', () => {
  ...
  try {
    const { stderr } = runHook(p)
    assert.match(stderr, /self-narration-nudge/)
  } finally {
    cleanup()
  }
})
...
test('self-narration: flags an unprompted status recap', () => {
  ...
  try {
    assert.match(runHook(p).stderr, /self-narration-nudge/)
  } finally {
    cleanup()
  }
})
