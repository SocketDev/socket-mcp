// node --test specs for the uncodified-lesson-nudge hook.
//
...
function makeTranscript(toolUses: Array<Record<string, unknown>>): string {
  const dir = mkdtempSync(path.join(tmpdir(), 'uncodified-lesson-nudge-'))
...
test('FIRES on an enforceable, uncited memory write', async () => {
  ...
  assert.match(r.stderr, /uncodified-lesson-nudge/)
