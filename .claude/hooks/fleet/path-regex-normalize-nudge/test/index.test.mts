/**
 * @file node --test specs for the path-regex-normalize-nudge hook. Stop hook
...
const NUDGE = /\[path-regex-normalize-nudge]/

type Result = { code: number; stderr: string }

function makeTranscript(...turns: Array<Record<string, unknown>>): string {
  const dir = mkdtempSync(path.join(tmpdir(), 'path-regex-nudge-'))
...
test('PASS-THROUGH: empty transcript exits 0 with no nudge', async () => {
  const dir = mkdtempSync(path.join(tmpdir(), 'path-regex-nudge-empty-'))
