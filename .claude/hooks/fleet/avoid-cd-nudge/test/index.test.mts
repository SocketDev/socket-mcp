// node --test specs for the avoid-cd-nudge hook.
//
...
function makeTranscript(userText: string): string {
  const dir = mkdtempSync(path.join(tmpdir(), 'avoid-cd-nudge-'))
...
const NUDGE = /\[avoid-cd-nudge\] Bash command contains a bare `cd/
