...
async function main(): Promise<void> {
  ...
  process.stderr.write(
    `[parallel-agent-on-stop-nudge] ${foreign.length} dirty path(s) this session did not author and that changed recently — likely another agent on the same checkout:\n`,
  )
...
main().catch(e => {
  process.stderr.write(
    `[parallel-agent-on-stop-nudge] hook bug — fail-open. ${e instanceof Error ? e.message : String(e)}\n`,
  )
})
