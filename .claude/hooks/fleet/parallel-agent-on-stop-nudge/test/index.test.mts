/**
 * @file Unit tests for parallel-agent-on-stop-nudge hook. Stop hook, always
...
test('reminds when a foreign dirty file exists (no transcript)', () => {
  ...
  assert.match(r.stderr, /parallel-agent-on-stop-nudge/)
...
test('silent on a clean repo', () => {
  const r = runHook({ cwd: repo })
  assert.equal(r.code, 0)
  assert.doesNotMatch(r.stderr, /parallel-agent-on-stop-nudge.*dirty/s)
})
