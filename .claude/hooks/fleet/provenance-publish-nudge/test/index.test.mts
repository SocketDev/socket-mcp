/**
 * @file Multi-case spec for provenance-publish-nudge. This is a Stop hook,
...
const STATE_REL = path.join('.claude', 'state', 'provenance-nudge.last')
...
function makeBareDir(): string {
  return mkdtempSync(path.join(tmpdir(), 'provenance-nudge-'))
}
...
function makeRepo(options: {
  ...
  const dir = mkdtempSync(path.join(tmpdir(), 'provenance-nudge-'))
  git(dir, 'init', '-q')
...
test('no package.json in cwd → exit 0, no nudge (pass-through)', async () => {
  ...
  assert.doesNotMatch(result.stderr, /provenance-publish-nudge/)
})
...
test(
  ...
  async () => {
    ...
    assert.match(
      result.stderr,
      /\[provenance-publish-nudge\] lodash@1\.0\.0 is published but missing:/,
    )
