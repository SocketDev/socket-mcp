...
- `extension-build-current-nudge` — pairs `tools/.../extension/src/**` edits with a build
...
- `yakback-nudge` — merged Stop scan: teacher-tone comments + "the user" naming + speed-vs-depth choice menus + self-narration (status-recap padding, "now let me" openers, hedges, apology-padding); per-group disable env vars preserved
