...
- **Hook** (`.claude/hooks/fleet/<name>/`): edit-time and tool-time enforcement. A `-guard` (PreToolUse, exit 2) BLOCKS a dangerous action before it happens. A `-nudge` (Stop or PreToolUse, exit 0) NUDGES when you can't hard-block. One surface per concern, never both a guard and a reminder for the same thing.
