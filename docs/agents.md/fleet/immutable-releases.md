...
🚨 **Workflow rule:** every release workflow under `.github/workflows/` that publishes a GitHub Release MUST use the 3-step pattern. The single-call `gh release create <tag> <files>` form is forbidden in fleet release workflows. Enforced at edit time by `.claude/hooks/fleet/immutable-release-guard/`, which blocks an Edit/Write that introduces a `gh release create` without `--draft` into a workflow YAML (bypass `Allow immutable-release-pattern bypass`). To audit committed workflows: `grep -rn "gh release create" .github/workflows/ | grep -v "\-\-draft"`.
