...
test('buildPurl produces correct PURLs across all ecosystems', async t => {
  ...
  await t.test('openvsx rewrites to vscode type with repository_url and platform', () => {
    assert.strictEqual(
  ...
  await t.test('chrome extensions pass through with 1.0.0 kept as real version', () => {
    assert.strictEqual(
      buildPurl('chrome', 'gighmmpiobklfepjocnamgkkbiglidom', '1.0.0'),
      'pkg:chrome/gighmmpiobklfepjocnamgkkbiglidom@1.0.0',
    )
  })
