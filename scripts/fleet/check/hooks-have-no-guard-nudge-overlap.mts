...
// and a `-nudge` for the same thing. Duplication crept in once (the prose
...
export function findOverlap(names: readonly string[]): OverlapReport {
  ...
  for (let i = 0, { length } = names; i < length; i += 1) {
    ...
    if (name.endsWith('-guard')) {
      ...
      reminders.add(name.slice(0, -'-nudge'.length))
    }
  }
...
function main(): void {
  ...
  if (exactCollisions.length) {
    logger.fail(
      '[check-hooks-have-no-guard-nudge-overlap] same-concern reminder + guard:',
    )
  ...
  if (!quiet && !exactCollisions.length) {
    logger.success(
      `[check-hooks-have-no-guard-nudge-overlap] no same-concern reminder/guard duplicates across ${names.length} hooks.`,
    )
  }
}
