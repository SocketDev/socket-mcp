 ...
 *   with the ScriptMeta second argument, imported from the shared runner
 *   itself. Four defects are flagged, scanning every `.mts` under
 *   scripts/fleet/ + scripts/repo/:
 *
 *   1. no-run-main — the file carries an entry guard
 *      (`isMainModule(import.meta.url)` / `import.meta.main`, as an
 *      `IfStatement` OR a top-level `<guard> && <call>` short-circuit) whose
 *      body never calls the shared runner, so a help request runs the
 *      script's side effect instead of printing usage. The fix is
 ...
 *      runMain(main, SCRIPT_META) }`.
 *   4. no-json — the guard's `runMain(...)` call SHAPE looks compliant (two
 *      arguments), but `runMain` is SHADOWED: a local `function runMain() {}`
 *      / `const runMain = …` declaration, or an import of `runMain` from
 *      somewhere other than `_shared/run-main.mts`. The shadow matches
 *      defects 1-3's identifier-only check yet does not answer `--describe
 *      --json` / `--json`, because that support lives in the shared
 *      runner's own code, not in anything the call site can express. The
 *      fix is importing the real one: `import { runMain } from
 *      '../_shared/run-main.mts'`.
 *
 ...
 *   guard `IfStatement`s, top-level `&&`-guarded `ExpressionStatement`s, and
 *   `ImportDeclaration`s cross into JS. This is the standard-things/esm
 ...
 */
export type SelfDescribeDefect =
  | 'no-run-main'
  | 'no-meta'
  | 'runs-on-import'
  | 'no-json'
...
export interface EstreeNode {
  ...
  readonly specifiers?: readonly EstreeNode[] | undefined
  readonly source?: EstreeNode | undefined
  readonly imported?: EstreeNode | undefined
  readonly local?: EstreeNode | undefined
...
/**
 * The `runMain(...)` call an expression resolves to, unwrapping a
 * `void`-prefixed call; `undefined` for anything else — including a call to
 * some OTHER function, since a guard body that never reaches the shared
 * runner is exactly what defect 1 (no-run-main) reports. Pure — exported for
 * tests and shared by both guard shapes below.
 */
export function runMainCallExpr(
  expr: EstreeNode | undefined,
...
/**
 * The `runMain(...)` call inside one guard-body statement; `undefined` when
...
export function runMainCallOf(stmt: EstreeNode): EstreeNode | undefined {
  ...
  return runMainCallExpr(stmt.expression)
}

/**
 * The verdict for one `runMain(...)` call site, guard `IfStatement` or
 * short-circuit alike: `undefined` when it carries a meta argument, `no-meta`
 * when it does not, `no-run-main` when the call resolved to nothing at all
 * (not `runMain`, or no call reached this site). Shared by {@link
 * guardVerdict} and the short-circuit `&&`-guard scan so both shapes rank
 * identically.
 */
export function runMainCallVerdict(
...
export function guardVerdict(
  ...
  for (const inner of body) {
    ...
    if (call) {
      return runMainCallVerdict(call)
    }
  }
  return 'no-run-main'
}
...
/**
 * The guarded action of a top-level short-circuit guard — `<entry-guard> &&
 * <action>` (either operand order) as its own `ExpressionStatement`, with no
 * `IfStatement` at all: `if (isMainModule(import.meta.url)) { main() }` and
 * `isMainModule(import.meta.url) && main()` are the same guard, spelled two
 * ways, and a call site that never uses `if` is not thereby out of scope.
 * `undefined` when neither side is an entry-guard condition, so an ordinary
 * `a && b()` is never mistaken for one. Pure — exported for tests.
 */
export function shortCircuitGuardAction(
  expr: EstreeNode,
): EstreeNode | undefined {
  if (expr.type !== 'LogicalExpression' || expr.operator !== '&&') {
    return undefined
  }
  if (expr.left !== undefined && isEntryGuardCondition(expr.left)) {
    return expr.right
  }
  if (expr.right !== undefined && isEntryGuardCondition(expr.right)) {
    return expr.left
  }
  return undefined
}
...
// The source suffix a `runMain` import must end with to be the shared
// runner. Suffix, not exact match, because a repo root and a fleet member
// checkout put a different number of `../` segments in front of the same
// relative path.
const SHARED_RUN_MAIN_SUFFIX = '_shared/run-main.mts'

/**
 * True when `text`'s top-level scope binds the identifier `runMain` to
 * something OTHER than the shared runner: a local `function runMain() {}` /
 * `const runMain = …` declaration, or an import of `runMain` whose source
 * does not end with `_shared/run-main.mts`. The call-site checks above only
 * see an IDENTIFIER named `runMain`; a shadowed one passes them by
 * coincidence of name while answering neither `--describe --json` nor
 * `--json`, since that support lives in the shared runner's OWN code, not
 * in anything visible at the call site. A file that never binds the name at
 * all is not flagged here — an unresolvable `runMain` reference is either a
 * real bug the type/lint gates already own, or (as in most of this file's
 * own test fixtures) a literal snippet never meant to run standalone. Pure —
 * exported for tests.
 */
export function hasShadowedRunMain(text: string): boolean {
  let shadowed = false
  walkRecursive(text, {
    FunctionDeclaration(node: AcornNode) {
      if ((node as unknown as EstreeNode).id?.name === 'runMain') {
        shadowed = true
      }
    },
    VariableDeclaration(node: AcornNode) {
      const decl = node as unknown as EstreeNode
      for (const d of decl.declarations ?? []) {
        if (d.id?.name === 'runMain') {
          shadowed = true
        }
      }
    },
    ImportDeclaration(node: AcornNode) {
      ...
      for (const spec of decl.specifiers ?? []) {
        if (spec.local?.name !== 'runMain') {
          continue
        }
        const source = decl.source?.value
        if (
          typeof source !== 'string' ||
          !source.endsWith(SHARED_RUN_MAIN_SUFFIX)
        ) {
          shadowed = true
        }
      }
...
export function classifyEntrySource(
  ...
  walkRecursive(text, {
    ...
    ExpressionStatement(node: AcornNode) {
      const stmt = node as unknown as EstreeNode
      const action =
        stmt.expression === undefined
          ? undefined
          : shortCircuitGuardAction(stmt.expression)
      if (action !== undefined) {
        verdicts.push(runMainCallVerdict(runMainCallExpr(action)))
      }
    },
  })
  ...
  if (!verdicts.includes(undefined)) {
    return verdicts.includes('no-run-main') ? 'no-run-main' : 'no-meta'
  }
  // At least one guard's call shape is compliant. Confirm the `runMain` it
  // calls is not shadowed by a local declaration or a differently-sourced
  // import — --json support lives entirely in the shared runner's own code,
  // so an identifier-only match is not enough to prove the call answers it.
  return hasShadowedRunMain(text) ? 'no-json' : undefined
}
...
export function main(): number {
  ...
  const noJson = findings.filter(f => f.kind === 'no-json')
  ...
  if (noJson.length > 0) {
    logger.error(
      `entry-scripts-are-self-describing: ${noJson.length} entry script(s) call a runMain that is not the shared runner, so --describe --json / --json are not answered.`,
    )
    logger.error(
      "  Where: a guard's runMain(main, meta) call whose runMain binding is locally declared or imported from somewhere other than the shared runner.",
    )
    logger.error(
      '  Saw: an identifier named runMain with two arguments; wanted: the real one, since --json support lives in its code, not in the call site.',
    )
    logger.error(
      "  Fix: `import { runMain } from '../_shared/run-main.mts'` (adjust the relative path to your file).",
    )
    for (let i = 0, { length } = noJson; i < length; i += 1) {
      logger.error(`  • ${noJson[i]!.file}`)
    }
  }
  return 1
}
