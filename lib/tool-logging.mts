...
import { debug, logger } from './logger.mts'
import {
  emitAuditEvent,
  extractResources,
  maskArgs,
...
import type { ToolCallResult, ToolHandlerExtra } from './tool-types.mts'
 ...
 * `tool-audit.mts`. The access token rides on `extra.authInfo` and is never
 * logged raw — the audit entry carries a truncated SHA-256 hash.
 *
 * Applied centrally inside `dispatchToolCall` (server.mts) so every tool gets
