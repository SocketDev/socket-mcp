 ...
 * canonical Socket form (e.g. `pkg:vscode/meta/pyrefly@1.0.0?repository_url=...`).
 */
export function buildPurl (
  ...
  } else if (ecoLower === 'maven' && (depname.includes(':') || depname.includes('/'))) {
  ...
  } else if ((ecoLower === 'openvsx' || ecoLower === 'vscode') && depname.includes('/')) {
    // VS Code and Open VSX extensions are identified as `publisher/extension`.
    const slash = depname.indexOf('/')
    namespace = depname.slice(0, slash)
    name = depname.slice(slash + 1)
  } else {
  ...
  // `1.0.0` is a stale model-default for ecosystems where the model didn't know the
  // version (npm/pypi historically). For ecosystems whose extensions/packages genuinely
  // publish 1.0.0 (e.g. openvsx, chrome), treat it as a real version.
  const placeholderEcosystems = new Set(['npm', 'pypi'])
  const isPlaceholderVersion = version === 'unknown'
    || !version
