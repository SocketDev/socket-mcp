 ...
 *
 *   - `assertSafeHttpUrl` — the SSRF + transport guard for operator/issuer-
 *     supplied URLs. Contract: returns a `URL` for public https URLs; throws an
 *     `Error` (never any other type) for non-http(s) schemes, unparseable
 *     input, cleartext http on a public host, and loopback/private/link-local
 *     hosts (unless `allowLocalhost` opens the gate for `localhost`/`127.0.0.1`
 *     over either scheme).
...
// URLs the guard must ACCEPT (public https).
const publicHttpsUrl = publicHost.map(h => `https://${h}/`)

// Cleartext URLs on a public host, which the guard must REFUSE — they would
// carry a bearer token and the introspection client secret in the clear.
const publicCleartextUrl = publicHost.map(h => `http://${h}/`)
...
describe('lib/http-helpers assertSafeHttpUrl (fuzz)', () => {
  // Public https URLs pass and return a URL whose hostname is preserved.
  test('accepts public https URLs and returns a URL', () => {
    fc.assert(
      fc.property(publicHttpsUrl, raw => {
  ...
  // Cleartext http on a public host is refused whether or not the localhost
  // gate is open — that gate covers local hosts only.
  test('rejects cleartext http on a public host', () => {
    fc.assert(
      fc.property(publicCleartextUrl, fc.boolean(), (raw, allowLocalhost) => {
        expect(() => assertSafeHttpUrl(raw, 'issuer', allowLocalhost)).toThrow(
          /must use https on a public host/u,
        )
      }),
    )
  })
