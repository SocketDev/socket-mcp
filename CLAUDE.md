...
- Fleet repos use Conventional Commits `<type>(<scope>): <description>`, lowercase, with NO AI attribution; non-fleet commits and GitHub PRs/issues follow that repo's rules. [`commit-cadence-format`](docs/fleet/agents.md/commit-cadence-format.md)
...
- Release App writes repo content. PR App changes issues and PRs. Both are organization-wide. [`token-hygiene`](docs/fleet/agents.md/token-hygiene.md)
...
- 🚨 Stdio is framed JSON-RPC on stdout - diagnostics go to stderr via `logger` from `lib/logger.ts`, never `console.log`. [`architecture`](docs/repo/agents.md/architecture.md)
- 🚨 Tool descriptions and input schemas are wire contract - changing one breaks clients, so bump the server version with it. [`architecture`](docs/repo/agents.md/architecture.md)
- 🚨 Both transports take the server FACTORY, so `createConfiguredServer()` must return a fresh `Server` on every call. [`architecture`](docs/repo/agents.md/architecture.md)
- Tests live under `test/{unit,integration,e2e,fleet}/` as `*.test.mts` mirroring the module they cover, never beside `lib/`. [`architecture`](docs/repo/agents.md/architecture.md)
- 🚨 Never put `--` before a test path - the path stops being positional and the run widens to the whole suite. [`architecture`](docs/repo/agents.md/architecture.md)
- Mock the Socket API in tests; only `test/e2e/` reaches the network, run by `pnpm run test:e2e` behind a token. [`architecture`](docs/repo/agents.md/architecture.md)
- Layout, the SDK package split, the tool roster, and the command table live in [`architecture`](docs/repo/agents.md/architecture.md).
